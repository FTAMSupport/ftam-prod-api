...
TaxSchema.methods.toPostJSON = function () {
  return {
    name: this.name,
...
TaxSchema.plugin(uniqueValidator, {message: 'is already taken'});
...
TaxSchema.methods.slugify = function() {
  this.slug = slug(this.title) + '-' + (Math.random() * Math.pow(36, 6) | 0).toString(36);
};

TaxSchema.methods.updateFavoriteCount = function() {
  var article = this;
...
TaxSchema.methods.toJSONFor = function(user){
  return {
    slug: this.slug,
    title: this.title,
    description: this.description,
